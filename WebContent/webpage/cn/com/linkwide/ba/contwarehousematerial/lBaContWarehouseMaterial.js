...
function openSelectWarehouse(){
	...
	$.dialog({
		content: 'url:lBaWarehouseController.do?listForSelect', 
		zIndex: 3100, 
		title: '仓库', 
		lock: true, 
		width: 750, 
		...
		opacity: 0.4, 
		button: [
		         {name: '确定', callback: callbackWarehouseSelect, focus: true},
		         {name: '取消', callback: function (){}}]
	}).zindex();
}

function callbackWarehouseSelect(){
	var iframe = this.iframe.contentWindow;
	var rowsData = iframe.$('#lBaWarehouseList4Select').datagrid('getSelections');
	...
	$("#warehouseCode").val(rowsData[0].warehouseCode);
	$("#warehouseId").val(rowsData[0].id);
	$("#warehouseName").val(rowsData[0].warehouseName);
}
