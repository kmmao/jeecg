...
<link rel="stylesheet" type="text/css"
	href="plug-in/easyui/portal/portal.css">
...
/* 放大镜样式 */
.icon-searchN {
	background: url('plug-in/easyui/themes/icons/search.png') no-repeat
		right 10px center;
}
</style>
<script type="text/javascript">
  ...
  //设置定时器的时间
  function loadTime(){
	  var url="taskTimeconfigController.do?updateLoadTimeSelect";
	  var taskId=$("#id").val();
	  url+="&taskId="+taskId;
	  $.dialog({
...
<body>
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		layout="table" action="backTaskDepController.do?doUpdate" tiptype="1">
		<input id="id" name="id" type="hidden" value="${backTaskDepPage.id }">
		...
		<div id="content">
			<div id="tab_bar">
				<ul>
					<li id="tab1" onclick="myclick(1)"><a
						href="javascript:void(0)" class="tabs-inner"
						style="color: #0f09ea; font-weight: 800;"> <span
							class="tabs-title">常规设置</span> <span class="tabs-icon"></span>
					</a></li>
					<li id="tab2" onclick="myclick(2)"><a
						href="javascript:void(0)" class="tabs-inner"> <span
							class="tabs-title">定时配置</span> <span class="tabs-icon"></span>
					</a></li>
				</ul>
			</div>
			<div class="tab_css" id="tab1_content" style="display: block">
				<table style="width: 100%;" cellpadding="0" cellspacing="1"
					class="formtable">
					<tr>
						<td align="right"><label class="Validform_label"> <i
								class="interval">*</i>任务名称:
						</label></td>
						<td class="value"><input id="taskName" name="taskName"
							type="text" value='${backTaskDepPage.taskName}'
							style="width: 75%" class="inputxt" dataType="*" nullmsg="请填写任务名称">
							<span class="Validform_checktip"></span> <label
							class="Validform_label" style="display: none;">任务名称</label></td>
						<td align="right"><label class="Validform_label"> <i
								class="interval">*</i>任务状态 :
						</label></td>
						<td class="value"><select id="taskState" name="taskState"
							style="width: 77%" dataType="*" nullmsg="请填写任务状态">
								<option value="">请选择</option>
								<option value="活动"
									<c:if test="${backTaskDepPage.taskState=='活动'}">selected="selected" </c:if>>活动</option>
								<option value="休眠"
									<c:if test="${backTaskDepPage.taskState=='休眠'}">selected="selected" </c:if>>休眠</option>
						</select> <span class="Validform_checktip"></span> <label
							class="Validform_label" style="display: none;">任务状态 </label></td>
					</tr>
					<tr>
						<td align="right"><label class="Validform_label"> <i
								class="interval">*</i>任务类型:
						</label></td>
						<td class="value"><select id="taskType" style="width: 77%;"
							dataType="*" nullmsg="请填写任务类型" onchange="typeChange()">
								<option value="">请选择</option>
								<c:forEach items="${taskTypeReList }" var="task">
									<option value="${task.businesPlugIn }"
										<c:if test="${task.taskName==backTaskDepPage.taskType}">selected="selected" </c:if>>${task.taskName }</option>
								</c:forEach>
						</select> <span class="Validform_checktip"></span> <label
							class="Validform_label" style="display: none;">任务类型</label> <input
							name="taskType" hidden="true"></td>
						<td align="right"><label class="Validform_label">
								业务插件: </label></td>
						<td class="value"><span id="plugName" style="color: gray;">${backTaskDepPage.plugName}</span>
							<input name="plugName" hidden="true"
							value="${backTaskDepPage.plugName}"> <span
							class="Validform_checktip"></span> <label class="Validform_label"
							style="display: none;">业务插件</label></td>
					</tr>
					<tr>
						<td align="right"><label class="Validform_label"> 描述:
						</label></td>
						<td class="value" colspan="3"><textarea rows="10" cols="30"
								id="taskDesc" name="taskDesc"
								value='${backTaskDepPage.taskDesc}' style="width: 75%">
					     	 
					     	 </textarea> <span class="Validform_checktip"></span> <label
							class="Validform_label" style="display: none;">描述</label></td>
					</tr>
					<tr>
						<td colspan="4"><span>实参设置</span></td>
					</tr>
					<tr>
						<td colspan="4">
							<table style="width: 100%;" border="1" cellpading="0"
								cellspacing="0">
								<tr>
			...
			<div class="tab_css" id="tab2_content">
				<table id="formtable_t" style="width: 100%;" cellpadding="0"
					cellspacing="1" class="formtable">
					<tr>
						<td align="right" width="100px"><label
							class="Validform_label"> <i class="interval">*</i>执行策略:
						</label></td>
						<td class="value"><select id="executWay" name="executWay"
							datatype="*" onchange="changWay()">
								<option value="">请选择</option>
								<option value="立即执行"
									<c:if test="${backTaskDepPage.executWay=='立即执行'}">selected="selected" </c:if>>立即执行</option>
								<option value="定时"
									<c:if test="${backTaskDepPage.executWay=='定时'}">selected="selected" </c:if>>定时</option>
						</select> <span class="Validform_checktip"></span> <label
							class="Validform_label" style="display: none;">执行策略</label></td>
					</tr>
					<tr id="happenTimeTd" style="display: none;">
						<td align="right"><label class="Validform_label">
								发生时间: </label></td>
						<td class="value"><input id="happenTime" name="happenTime"
							type="text" value="${backTaskDepPage.happenTime}"
							class="inputxt icon-searchN" readonly="readonly"
							onclick="loadTime()"> <span class="Validform_checktip"></span>
...
<script
	src="webpage/cn/com/linkwide/cost/custom/taskcenter/backtask/backTaskDep.js"></script>