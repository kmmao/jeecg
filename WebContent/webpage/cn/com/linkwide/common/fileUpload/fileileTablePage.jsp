...
<script type="text/javascript">
    		...
    		success : function(data) {
    			 ...
    			 if(data.indexOf("失败")>0){ 
    				 ...
    				 var tr  =  "<tr id=\"fileList"+d.obj.id+"\" datagrid-row-index=\"0\" class=\"datagrid-row datagrid-row-selected datagrid-row-over\" style=\"height: 51px;\">";
    			    	...
    			    	tr +=  "<td field=\"filedowload\"><div style=\"height:auto;\" class=\"datagrid-cell datagrid-cell-c1-opt\"><a href=\"${realPath}"+d.obj.filePath+"\" target=\"_blank\" class=\"ace_button\" onclick=\"\">  <i class=\" fa fa-download\"></i>下载</a>&nbsp;</div></td>";
    			    	...
    			    	tr +=  "</tr>";
    			 	 $("#add_vendorFileDict_table").append(tr);
    			 	resetTrNum('add_vendorFileDict_table');
    			 }
    		}
    	});
    }
 ...
 function editObj(id) {
	     ...
	     var hostport=document.location.host;
	  	 window.open("WebOffice://|Officectrl|http://"+hostport+"/hrp/webOfficeController.do?goEdit&sessionId="+sonId+"&id="+id,"_parent")
	    //openwindow('文档修改','webOfficeController.do?goEdit&isEdit=1','lPuDemandApplyList',700,500)
	    	//stringId =trId;
	   
	  }

 function uploadSuccess(d,file,response){ 
     /*  var url = d.attributes.url; 
      ...
      var tr  =  "<tr id=\"fileList"+id+"\" datagrid-row-index=\"0\" class=\"datagrid-row datagrid-row-selected datagrid-row-over\" style=\"height: 51px;\">";
		...
		tr +=  "<td field=\"fileName\"><div style=\"height:auto;\" class=\"datagrid-cell datagrid-cell-c1-fileName\">"+getTimeStamp()+"</div></td>";
	  	tr +=  "<td field=\"filedowload\"><div style=\"height:auto;\" class=\"datagrid-cell datagrid-cell-c1-opt\"><a  href=\"${realPath}"+filePath+"\" target=\"_blank\" class=\"ace_button\" onclick=\"\">  <i class=\" fa fa-download\"></i>下载</a>&nbsp;</div></td>";
	  	if("${isShow}" == "1"){
	  		tr +=  "<td field=\"filedel\"><div style=\"height:auto;\" class=\"datagrid-cell datagrid-cell-c1-opt\"><a href=\"#\" class=\"ace_button\" onclick=\"editObj('"+id+"')\">  <i class=\" fa fa-search-plus\"></i>预览</a>&nbsp;</div></td>";
	  	}
...
<c:if test="${isEdit  == 1 && tableId !='' && tableId !=null }">
  <table cellpadding="0" cellspacing="0" class="formtable">
   <tbody>
    <tr height="60px">
     <td align="right" width="20%">
       <label class="Validform_label">请选择需要上传的文件：</label>
     </td>
     <td class="value" width="140px">
      <t:upload name="instruction" fileSizeLimit="100MB" dialog="false" multi="false" extend="" queueID="instructionfile" view="false" auto="true" uploader="cgUploadController.do?ajaxSaveFile&modName=${modName}&tableName=${tableName}&tableId=${tableId}" onUploadSuccess="uploadSuccess"  id="instruction" formData="documentTitle"></t:upload>
     </td>
...
<table border="0" cellpadding="2" cellspacing="1" id="vendorFileDict_table" width="100%">
	...
	<td field="filePath"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-filePath">文件展示</td>
	<td field="fileName"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-fileName">文件名称</div></td>
	<td field="fileDate"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-fileName">上传日期</div></td>
	<td field="filedowload"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-filePath">下载</td>
	<c:if test="${isShow  == 1 }">
	<td field="filedel"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-filePath">预览</td>
	</c:if>
	<c:if test="${isEdit  == 1  }">
	<td field="filedel"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-filePath">删除</td>
	</c:if>
 	 </tr>
	<tbody id="add_vendorFileDict_table">
	<c:if test="${fn:length(fileDictList)  > 0 }">
		<c:forEach items="${fileDictList}" var="poVal" varStatus="stuts">
			<tr id="fileDictList${stuts.index}" datagrid-row-index="0" class="datagrid-row datagrid-row-selected datagrid-row-over" style="height: 51px;">
				...
				<td field="filePath"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-filePath"><img width="50" class="img" height="50" border="0" onmouseover="tipImg(this)" onmouseout="moveTipImg()" src="${realPath}${poVal.filePath }"></div></td>
			 	<td field="fileName"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-fileName">${poVal.fileName }</div></td>
			 	<td field="fileDate"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-fileName"><fmt:formatDate value='${poVal.createDate }' type="date" pattern="yyyy-MM-dd hh:mm:ss"/></div></td>
				<td field="filedowload"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-opt"><a href="${realPath}${poVal.filePath }" target="_blank" class="ace_button" onclick="">  <i class=" fa fa-download"></i>下载</a>&nbsp;</div></td>
				<c:if test="${isShow  == 1 }">
				<td field="filedel"><div style="height:auto;" class="datagrid-cell datagrid-cell-c1-opt"><a href="#" class="ace_button" onclick="editObj('${poVal.id }')">  <i class=" fa fa-search-plus"></i>预览</a>&nbsp;</div></td>
				</c:if>	
				<c:if test="${isEdit  == 1  }">
