...
<body>
	<div class="easyui-layout" fit="true" scroll="no">
		<div  data-options="region:'west',title:'单位管理',split:true" style="width:300px;overflow: auto;">
...
function loadTree() {
	...
	jQuery.ajax({  
        ...
        type: 'POST',  
        dataType : "json",  
        url: 'tSCompanyController.do?getTreeData',//请求的action路径  
	...
	var ztreeCreator = new ZtreeCreator('companyTree',"tSCompanyController.do?getTreeData",zNodes)
 			.setCallback({onClick:zTreeOnLeftClick,onRightClick:zTreeOnRightClick})
 			.initZtree({},function(treeObj){companyTree = treeObj});
...
function zTreeOnLeftClick(event, treeId, treeNode) {
	...
	var url = "tSCompanyController.do?goUpdate&id=" + curSelectNode.id;
...
function addNode() {
	...
	var url = "tSCompanyController.do?goAdd&id=" + selectNode.id;
	$("#listFrame").attr("src", url);
...
function delNode() {
	...
	var url = "tSCompanyController.do?doDel&id="+nodeId;
	if(!confirm('温馨提示', '确定删除该单位以及该单位下的所有子组织？')){
		return;
	}
	jQuery.ajax({  
        async : false,  
        ...
        success:function(data){ //请求成功后处理函数。
		    if(data.success){
		    	companyTree.removeNode(selectNode);
...
function getSelectNode() {
	...
	var nodes = companyTree.getSelectedNodes();
	var node = nodes[0];
	return node;
};
</script>
