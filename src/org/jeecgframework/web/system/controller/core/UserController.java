...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        if(StringUtil.isNotEmpty(ResourceUtil.getSessionUser().getSysCompanyCode())){
			...
			cq.notEq("id", ResourceUtil.getSessionUser().getId());
		}
        ...
        if(!"system".equals(ResourceUtil.getSessionUser().getUserName())){
        	TSBaseUser u = systemService.findUniqueByProperty(TSBaseUser.class, "userName", "system");
        	cq.add(Restrictions.not(Restrictions.in("id", u.getId().split(","))));
        }
        this.systemService.getDataGridReturn(cq, true);
        List<TSUser> cfeList = new ArrayList<TSUser>();
        for (Object o : dataGrid.getResults()) {
            if (o instanceof TSUser) {
            	if(!"system".equals(((TSUser) o).getUserName())){
	...
	public AjaxJson delete(TSUser user, @RequestParam String deleteType, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		TSUser u = systemService.get(TSUser.class, user.getId());
		if("group_admin".equals(u.getUserName())){
			j.setMsg("集团管理员[group_admin]不可删除");
			return j;
		}
		if("system".equals(u.getUserName())){
			j.setMsg("平台管理员[system]不可删除");
			return j;
		}
		if("admin".equals(u.getUserName())){
			j.setMsg("超级管理员[admin]不可删除");
			return j;
		}
	...
	public AjaxJson del(TSUser user, HttpServletRequest req) {
		String message = null;
		AjaxJson j = new AjaxJson();
		if("group_admin".equals(user.getUserName())){
			message = "集团管理员[group_admin]不可删除";
			j.setMsg(message);
			return j;
		}
		if("system".equals(user.getUserName())){
			message = "平台管理员[system]不可删除";
			j.setMsg(message);
			return j;
		}
		if("admin".equals(user.getUserName())){
...
**/	
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			if (users != null) {
				...
				user.setSysCompanyCode(ResourceUtil.getSessionUser().getSysCompanyCode());
	...
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			if(StringUtil.isEmpty(user.getSysCompanyCode()) && StringUtil.isNotEmpty(ResourceUtil.getSessionUser().getSysCompanyCode())){
				user.setSysCompanyCode(ResourceUtil.getSessionUser().getSysCompanyCode());
			}
		...
		req.setAttribute("user", user);
		req.setAttribute("sysCompanyCode", ResourceUtil.getSessionUser().getSysCompanyCode());
