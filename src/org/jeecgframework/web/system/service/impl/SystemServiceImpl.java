...
import cn.com.linkwide.ba.optioncontrol.entity.OptionControlEntity;
...
//		log.setTSUser(ResourceUtil.getSessionUser());
	...
	public Map<String, String> getAccordingToTheDign(String key) {
		List<OptionControlEntity> optionList = this.commonDao.findByProperty(OptionControlEntity.class, "modularIdentification", key);
		Map<String,String> map = new HashMap<String,String>();
		for (OptionControlEntity option : optionList) {
			map.put(option.getOptionIdentification(), option.getOptionParameter());
		}
		return map;
	}
	...
	public void updateAccordingToTheDign(Map<String, String> map, String key) {
		String hql = "FROM OptionControlEntity WHERE optionIdentification = ? AND modularIdentification = ?";
		// 循环查询并修改
		for(Entry<String, String> entry : map.entrySet()) {
			List<OptionControlEntity> optionList = commonDao.findHql(hql, entry.getKey(),key);
			if(optionList.size()!=0) {
				OptionControlEntity optionControl = optionList.get(0);
				if(StringUtil.isNotEmpty(optionControl)) {//不为null处理
//					if(StringUtil.isNotEmpty(optionControl.getOptionParameter()) && !optionControl.getOptionParameter().equals(entry.getValue())) {//不相等则更改
						optionControl.setOptionParameter(entry.getValue());
						commonDao.saveOrUpdate(optionControl);
//					}
				}
			}
		}
	...
	public void queryDataGrid(String sql, DataGrid dataGrid) {
		...
		Long count  = getCountForJdbc("select count(*) from ("+ sql.toString()+" ) queryview ");
		dataGrid.setResults(list);
		dataGrid.setTotal(Integer.valueOf(count.toString()));
		try{
			if(StringUtil.isNotEmpty(dataGrid.getFooter())){
				String footer = dataGrid.getFooter();
				String sumSql = "select ";
				for(String f :footer.split(",")){
					if(sql.indexOf("\""+f+"\"") > 0){
						sumSql += "SUM(\""+f+"\") AS \""+f+"\","; 
					}else{
						sumSql += "SUM("+f+") AS \""+f+"\","; 
				...
				sumSql +=" 1 from ("+sql+") queryview ";
				Map<String,Object> map  = findOneForJdbc(sumSql);
				footer ="";
				for (String key : map.keySet()) { 
					footer += key+":"+(map.get(key) == null ? 0.00 :map.get(key).toString())+",";
				}
				dataGrid.setFooter(footer+dataGrid.getFooterExtend());
			}
		}catch(Exception e){}
	}
}
