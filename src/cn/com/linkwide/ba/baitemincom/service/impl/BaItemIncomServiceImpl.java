package cn.com.linkwide.ba.baitemincom.service.impl;
...
@Service("baItemIncomService")
@Transactional
public class BaItemIncomServiceImpl extends CommonServiceImpl implements BaItemIncomServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(BaItemIncomEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(BaItemIncomEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(BaItemIncomEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(BaItemIncomEntity t){
		...
		map.put("budg_year", t.getBudgYear());
		...
		map.put("budg_source", t.getBudgSource());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,BaItemIncomEntity t){
 		...
 		sql  = sql.replace("#{budg_year}",String.valueOf(t.getBudgYear()));
 		...
 		sql  = sql.replace("#{budg_source}",String.valueOf(t.getBudgSource()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("ba_item_incom",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
