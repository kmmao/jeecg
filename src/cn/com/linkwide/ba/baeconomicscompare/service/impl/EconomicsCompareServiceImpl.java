package cn.com.linkwide.ba.baeconomicscompare.service.impl;
...
@Service("economicsCompareService")
@Transactional
public class EconomicsCompareServiceImpl extends CommonServiceImpl implements EconomicsCompareServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(EconomicsCompareEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(EconomicsCompareEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(EconomicsCompareEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(EconomicsCompareEntity t){
		...
		map.put("v_code", t.getVcode());
		map.put("v_name", t.getVname());
		map.put("acct_subj", t.getAcctSubj());
		map.put("dept_ec_code", t.getDeptEcCode());
		map.put("dept_ec_name", t.getDeptEcName());
		map.put("gov_code", t.getGovCode());
		map.put("gov_name", t.getGovName());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,EconomicsCompareEntity t){
 		...
 		sql  = sql.replace("#{v_code}",String.valueOf(t.getVcode()));
 		sql  = sql.replace("#{v_name}",String.valueOf(t.getVname()));
 		sql  = sql.replace("#{acct_subj}",String.valueOf(t.getAcctSubj()));
 		sql  = sql.replace("#{dept_ec_code}",String.valueOf(t.getDeptEcCode()));
 		sql  = sql.replace("#{dept_ec_name}",String.valueOf(t.getDeptEcName()));
 		sql  = sql.replace("#{gov_code}",String.valueOf(t.getGovCode()));
 		sql  = sql.replace("#{gov_name}",String.valueOf(t.getGovName()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("economics_compare",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
